...
function init (env, cb) {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key.startsWith('STORAGE_')) {
      envStorage[key.substring(8)] = value
    }
  }
  const driver = new DriverClass(envStorage)
  ...
  console.warn(`EXPERIMENTAL: loading custom storage driver '${DriverClass.name}' from '${packageName}'...`)
